...
export default function YouTubeScreen() {
  const { data: status } = useConnectionStatus("youtube");
  const disconnect = useDisconnectService();
  const accessToken = useAuthStore((s) => s.accessToken);

  const handleConnect = async () => {
    const url = `${API_URL}/api/auth/youtube/connect?source=mobile&token=${accessToken}`;
    await WebBrowser.openBrowserAsync(url);
  };

  const handleDisconnect = () => {
    Alert.alert("Disconnect", "Disconnect YouTube?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Disconnect",
        style: "destructive",
        onPress: () => disconnect.mutate("youtube"),
      },
    ]);
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="flex-row items-center border-b border-gray-200 bg-white px-4 py-3">
        ...
        <Text className="text-xl font-bold text-gray-900">YouTube</Text>
      </View>
        ...
        <View className="rounded-xl bg-white p-4 shadow-sm">
          <Text className="text-base text-gray-600">
            {status?.connected
              ...
              : "Connect YouTube to manage your subscriptions through Lightfriend"}
          </Text>
        </View>
