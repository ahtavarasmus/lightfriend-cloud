...
export default function WhatsAppScreen() {
  const { data: status } = useConnectionStatus("whatsapp");
  ...
  const handleDisconnect = () => {
    Alert.alert("Disconnect", "Are you sure you want to disconnect WhatsApp?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Disconnect",
        style: "destructive",
        onPress: () => disconnect.mutate("whatsapp"),
      },
    ]);
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="flex-row items-center border-b border-gray-200 bg-white px-4 py-3">
        ...
        <Text className="text-xl font-bold text-gray-900">WhatsApp</Text>
      </View>
      ...
      <View className="flex-1 px-4 pt-4">
        <View className="rounded-xl bg-white p-4 shadow-sm">
          <Text className="text-base text-gray-600">
            {status?.connected
              ? `Connected as ${status.username ?? "WhatsApp user"}`
              : "Connect your WhatsApp to receive messages through Lightfriend"}
          </Text>
        </View>
          ...
          <>
            {qrData ? (
              <QRCodeDisplay
                base64Data={qrData}
                isLoading={false}
                label="Scan this QR code with WhatsApp on your phone"
              />
            ) : (
              <Pressable
