...
export default function TeslaScreen() {
  const { data: status } = useConnectionStatus("tesla");
  const disconnect = useDisconnectService();
  const accessToken = useAuthStore((s) => s.accessToken);

  const handleConnect = async () => {
    const url = `${API_URL}/api/auth/tesla/connect?source=mobile&token=${accessToken}`;
    await WebBrowser.openBrowserAsync(url);
  };

  const handleDisconnect = () => {
    Alert.alert("Disconnect", "Disconnect Tesla?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Disconnect",
        style: "destructive",
        onPress: () => disconnect.mutate("tesla"),
      },
    ]);
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="flex-row items-center border-b border-gray-200 bg-white px-4 py-3">
        ...
        <Text className="text-xl font-bold text-gray-900">Tesla</Text>
      </View>
      ...
      <View className="flex-1 px-4 pt-4">
        <View className="rounded-xl bg-white p-4 shadow-sm">
          <Text className="text-base text-gray-600">
            {status?.connected
              ? "Tesla is connected"
              : "Connect Tesla to control your vehicle through Lightfriend"}
          </Text>
        </View>
